...
<?php  
	...
	if(isset($_POST['add']))
	{
		...
		$qry=mysqli_query($con,"UPDATE `route` SET `rid`='$rid',`depart_time`='$depart_time',`arr_time`='$arr_time',`distance`='$distance',`origin`='$origin',`destination`='$destination',`fare`='$fare' WHERE rid='$id'")or die(mysqli_error($con));
		if($qry==true)
		{
			echo "
			<script>
